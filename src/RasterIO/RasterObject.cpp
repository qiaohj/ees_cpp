...
int RasterObject::getXSize(){
	return xSize;
}
int RasterObject::getYSize(){
	return ySize;
}
...
float RasterObject::readByXY(int x, int y){
	if (x<0||x>=xSize||y<0||y>=ySize){
		return noData;
	}
	return *(valueArray + (y * xSize + x));
}
void RasterObject::XY2LL(int x, int y, double* longitude, double* latitude){
...
void RasterObject::LL2XY(double longitude, double latitude, int* x, int* y){
	*x = (int)((longitude - adfGeoTransform[0]) / adfGeoTransform[1]);
	*y = (int)((latitude - adfGeoTransform[3]) / adfGeoTransform[5]);
}
