...
static const char* kDaysAbbrev[7] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri",
        "Sat" };
static const char* kMonths[12] = { "January", "February", "March", "Apri",
        "May", "June", "July", "August", "September", "October", "November",
        "December" };
static const char* kMonthsAbbrev[12] = { "Jan", "Feb", "Mar", "Apr", "May",
...
} kTimeFormats[] = { { 1000.0f, ELPP_LITERAL("mis") }, { 1000.0f, ELPP_LITERAL(
        "ms") }, { 60.0f, ELPP_LITERAL("seconds") }, { 60.0f, ELPP_LITERAL(
        "minutes") }, { 24.0f, ELPP_LITERAL("hours") }, { 7.0f, ELPP_LITERAL(
        "days") } };
...
template<typename T>
static inline typename std::enable_if<std::is_pointer<T*>::value, void>::type safeDelete(
        T*& pointer) {  // NOLINT
    if (pointer == nullptr)
        return;
    delete pointer;
...
static inline void abort(int status, const std::string& reason =
        std::string()) {
...
#   endif  // _ELPP_COMPILER_MSVC || defined(_MSC_EXTENSIONS)
    ...
    static base::type::string_t formatTime(unsigned long long time,
            base::TimestampUnit timestampUnit) {  // NOLINT
    ...
    static inline unsigned long long getTimeDifference(
        ...
        if (timestampUnit == base::TimestampUnit::Microsecond) {
            return static_cast<unsigned long long>(static_cast<unsigned long long>(1000000
                    * endTime.tv_sec + endTime.tv_usec) -  // NOLINT
            ...
            return static_cast<unsigned long long>((((endTime.tv_sec
                    ...
                    + (endTime.tv_usec - startTime.tv_usec)) / 1000);  // NOLINT
        }
    }
            ...
            if (*format == base::consts::kFormatSpecifierChar) {
                ...
                case 'g':
                    buf = base::utils::Str::convertAndAddToBuff(msec,
                            msWidth->m_width, buf, bufLim);
                    continue;
                case 'F':  // AM/PM
                    buf = base::utils::Str::addToBuff(
                            (tInfo->tm_hour >= 12) ?
                                    base::consts::kPm : base::consts::kAm, buf,
                            bufLim);
                    continue;
                default:
                    continue;
...
protected:
    ...
    virtual inline void registerNew(const T_Key& uniqKey, T_Ptr* ptr)
            ELPP_FINAL {
        unregister(uniqKey);
...
private:
    virtual inline void deepCopy(
            const AbstractRegistry<T_Ptr, std::map<T_Key, T_Ptr*>>& sr)
                    ELPP_FINAL {
...
protected:
    ...
    virtual void updateDateFormat(std::size_t index,
            base::type::string_t& currFormat) ELPP_FINAL {  // NOLINT
...
public:
    ...
    inline const base::MillisecondsWidth& millisecondsWidth(Level level =
            Level::Global) {
        return getConfigByRef<base::MillisecondsWidth>(level,
                &m_millisecondsWidthMap, "millisecondsWidth");
    }
...
public:
    ...
    HitCounter(const HitCounter& hitCounter) :
            m_filename(hitCounter.m_filename), m_lineNumber(
                    hitCounter.m_lineNumber), m_hitCounts(
    ...
    inline void resetLocation(const char* filename,
            unsigned long int lineNumber) {  // NOLINT
        m_filename = filename;
...
public:
    explicit Storage(const LogBuilderPtr& defaultLogBuilder) :
            m_registeredHitCounters(new base::RegisteredHitCounters()), m_registeredLoggers(
                    ...
                    0x0), m_vRegistry(new base::VRegistry(0, &m_flags)), m_preRollOutCallback(
                    base::defaultPreRollOutCallback) {
    ...
    inline bool validateEveryNCounter(const char* filename,
            unsigned long int lineNumber, std::size_t occasion) {  // NOLINT
...
#else
                                m_logger->stream() << ": " << strerror(errno)
                                        << " [" << errno << "]";
#endif
                            }
                        }
...
// Logging from Logger class. Why this is here? Because we have Storage and Writer class available
#if _ELPP_VARIADIC_TEMPLATES_SUPPORTED
                    template<typename T, typename ... Args>
                    void Logger::log_(Level level, int vlevel, const char* s,
                    ...
                    template<typename T>
                    inline void Logger::log_(Level level, int vlevel,
                            const T& log) {
                        if (level == Level::Verbose) {
...
public:
    PerformanceTracker(const std::string& blockName,
            ...
            _CURRENT_FILE_PERFORMANCE_LOGGER_ID),
